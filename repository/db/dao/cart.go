...
// CreateCart 创建 cart pId(商品 id)、uId(用户id)
func (dao *CartDao) CreateCart(pID, uID uint) (cart *model.Cart, status int, err error) {
	// 查询有无该product
	cart, err = dao.GetCartById(pID, uID)
	// 空，该商品第一次加入
...
func (dao *CartDao) GetCartById(pID, uID uint) (cart *model.Cart, err error) {
	err = dao.DB.Model(&model.Cart{}).
		Where("user_id = ? AND product_id = ?", pID, uID).
		First(&cart).Error
